...
public class OfficeActivityMovement extends MapBasedMovement implements 
	...
	private List<Coord2> allOffices;
	
	private Coord2 lastWaypoint;
	private Coord2 officeLocation;
	private Coord2 deskLocation;
	...
	public OfficeActivityMovement(Settings settings) {
		...
		if (officeLocationsFile == null) {
			...
			try {
				allOffices = new LinkedList<Coord2>();
				List<Coord2> locationsRead = (new WKTReader()).
					readPoints(new File(officeLocationsFile));
				for (Coord2 coord : locationsRead) {
					SimMap map = getMap();
					Coord2 offset = map.getOffset();
	...
	public Coord2 getRandomCoorinateInsideOffice() {
		...
		return new Coord2(x_coord, y_coord);
	}
