...
public class ExternalMovement extends MovementModel {
	...
	private static final Coord2 DEF_INIT_LOC = new Coord2(0,0);
	...
	private static List<Tuple<String, Coord2>> initLocations;
	...
	private ExternalMovement(MovementModel mm) {
		...
		if (initLocations.size() > 0) { // we have location data left
			...
			Tuple<String, Coord2> initLoc = initLocations.remove(0); 
	 ...
	 */
	private void addLocation(Coord2 loc, double time) {
	...
	private static double readMorePaths() {
		List<Tuple<String, Coord2>> list = reader.readNextMovements();
		...
		for (Tuple<String, Coord2> t : list) {
