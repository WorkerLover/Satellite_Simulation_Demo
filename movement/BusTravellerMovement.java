...
public class BusTravellerMovement extends MapBasedMovement implements 
	...
	private Coord2 location;
	private Coord2 latestBusStop;
	...
	private Coord2 startBusStop;
	private Coord2 endBusStop;
	...
	public Coord2 getInitialLocation() {
		...
		List<Coord2> allStops = controlSystem.getBusStops();
		Coord2 closestToNode = getClosestCoordinate(allStops, location.clone());
	...
	public Path getPath() {
		...
		if (state == STATE_WAITING_FOR_BUS) {
			...
			List<Coord2> coords = nextPath.getCoords();
	...
	@Override
	public MapBasedMovement replicateDefault() {
	 ...
	 */
	private static Coord2 getClosestCoordinate(List<Coord2> allCoords, 
			Coord2 coord) {
		Coord2 closestCoord = null;
		double minDistance = Double.POSITIVE_INFINITY;
		for (Coord2 temp : allCoords) {
	...
	public void setNextRoute(Coord2 nodeLocation, Coord2 nodeDestination) {
		...
		Coord2 closestToNode = getClosestCoordinate(allStops, nodeLocation);
		Coord2 closestToDestination = getClosestCoordinate(allStops, 
				nodeDestination);
