...
public class WKTMapReader extends WKTReader {
	private Hashtable<Coord2, MapNode> nodes;
	...
	public WKTMapReader(boolean bidi) {
		...
		this.nodes = new Hashtable<Coord2, MapNode>();
	}
	 ...
	 */
	public Map<Coord2, MapNode> getNodesHash() {
		return this.nodes;
	}
	...
	public void addPaths(Reader input, int nodeType) throws IOException {
		...
		while((type = nextType()) != null) {
			if (type.equals(LINESTRING)) {
				...
				for (List<Coord2> list : parseMultilinestring()) {
					updateMap(list);
				}
			}
			else {
	 ...
	 */
	private void updateMap(List<Coord2> coords) {
	 ...
	 */
	private MapNode createOrUpdateNode(Coord2 c, MapNode previous) {
		MapNode n = null;
