/* JAT: Java Astrodynamics Toolkit
 * 
  Copyright 2012 Tobias Berthold

   Licensed under the Apache License, Version 2.0 (the "License");
   you may not use this file except in compliance with the License.
   You may obtain a copy of the License at

       http://www.apache.org/licenses/LICENSE-2.0

   Unless required by applicable law or agreed to in writing, software
   distributed under the License is distributed on an "AS IS" BASIS,
   WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
   See the License for the specific language governing permissions and
   limitations under the License.
 */

package jat.application.orbitviewer;

import core.DTNHost;

import java.util.ArrayList;
import java.util.List;

import jat.core.plot.plot.Plot3DPanel;
import jat.coreNOSA.algorithm.integrators.Printable;
import jat.coreNOSA.cm.TwoBody;
import jat.coreNOSA.cm.cm;


public class orbitviewerplot implements Printable {

	public double a = 10000.; // sma in km
	double e = 0.1; // eccentricity
	double i = 15; // inclination in degrees
	double raan; // right ascension of ascending node in degrees
	double w; // argument of perigee in degrees
	double ta; // true anomaly in degrees
	double max;

	Plot3DPanel plot;
	int steps = 200;
	double[][] XYZ = new double[steps][3];
	double[][] XYZ1 = new double[steps][3];
//	double[][] XYZ2 = new double[steps][3];
    double[][] points = new double[1][3];
    double[][] points1 = new double[1][3];
	int step;
	
//  private static suborbitviewer sov;
	private static Function func;

	public void add_scene(List<DTNHost>hosts,boolean flag) {
//		System.out.println("a " + a);
		//System.out.println("ta " + ta);
		TwoBody sat = new TwoBody(a, e, i, raan, w, ta);
//		TwoBody sat1 = new TwoBody(10000.0, 0.1, 20, raan, w, ta);
//		TwoBody sat2 = new TwoBody(9000.0, 0.1, 20, raan, w, ta);
		// find out the period of the orbit
		double period = sat.period();
	//	double period1 = sat1.period();
	//	double period2 = sat2.period();
		// set the final time = one orbit period
		double tf = period;
	//	double tf1 = period1;
	//	double tf2 = period2;
	//	System.out.println("tf1 "+ tf1);
		// set the initial time to zero
		double t0 = 0.0;
		sat.setSteps(steps);
	//	sat1.setSteps(steps);
//		sat2.setSteps(steps);
		// propagate the orbit
	//	sat.propagate(1000);
	//	sat1.propagate(tf1);

		step = 0;
		max = 0;
//		System.out.println("tf "+ tf);
		sat.propagate(t0, tf, this, true);
		
		points=new double[1][3] ;
		points[0][0]=sat.rv.x[0];
		points[0][1]=sat.rv.x[1];
		points[0][2]=sat.rv.x[2];
		
	//	System.out.println("points " + points[0][0]);
		
    //    System.out.println("MAX1 "+ max);
		
 
        /////////
     /* System.out.println("t0 "+ t0);
        System.out.println("tf1 "+ tf1);*/
   /*     step=0;
        max = 0;
        sat1.propagate(t0,tf1,this,false);
        
        points1=new double[1][3] ;
		points1[0][0]=sat1.rv.x[0];
		points1[0][1]=sat1.rv.x[1];
		points1[0][2]=sat1.rv.x[2];	*/
	
	//	System.out.println("XYZ1 " + XYZ1[198][0]);
	//	System.out.println("points1 " + points1[0][0]);
        
//		plot.addSpherePlot("earth", cm.earth_radius);
 /*     plot.addLinePlot("orbit1", XYZ1, true);
        plot.addScatterPlot("satellite1" ,1,5, points1);*/
 //       System.out.println("MAX2 "+ max);
        
  /*      step=0;
        max = 0;
        sat2.propagate1(t0,tf1,this,false);*/
		if(flag == false) {
			plot.addSpherePlot("earth",cm.earth_radius);
		    plot.addLinePlot("orbit", XYZ, true);
            plot.addScatterPlot("satellite" ,1,5, points);
       
		}
		
		if(flag == true) {
			plot.addSpherePlot("earth", cm.earth_radius);

			for(int i=0;i<hosts.size();++i) {
				plot.addLinePlot("orbit"+i, hosts.get(i).getXYZ(), true);
		
				plot.addScatterPlot("satellite"+i ,1, 5, hosts.get(i).get3Dpoints());
				if(this.max < hosts.get(i).getMAX()) {
					this.max = hosts.get(i).getMAX();
				}
				
				}
			this.func = new Function(hosts);
			new Thread(func).start();
/*			System.out.println("3dcoord "+ hosts.get(0).get3Dpoints()[0][0]+
					hosts.get(0).get3Dpoints()[0][1]+hosts.get(0).get3Dpoints()[0][2]);
			System.out.println("jingwei "+ hosts.get(0).get2Dpoints()[0][0]+hosts.get(0).get2Dpoints()[0][1]);*/
			System.out.println("test...");
//			System.out.println("testbefore");
		//	this.sov = new suborbitviewer(hosts);
/*			this.func = new Function(hosts);
			new Thread(func).start();
//			System.out.println("testafter");*/
		}
		
		
        
     	double size = max;
		plot.setFixedBounds(0, -size, size);
		plot.setFixedBounds(1, -size, size);
		plot.setFixedBounds(2, -size, size);
	}
	
	public void add_scene1(List<DTNHost> hosts,boolean flag) {
		for(int i=0;i<hosts.size();++i)
		{
			plot.addLinePlot("orbit"+"i", hosts.get(i).getXYZ(), true);
	        plot.addScatterPlot("satellite"+"i" ,1,5, hosts.get(i).get3Dpoints());
		}
	}
	
	public void make_plot(List<DTNHost>hosts,boolean flag) {
		// create your PlotPanel (you can use it as a JPanel) with a legend at
		// SOUTH
		plot = new Plot3DPanel("SOUTH");
		// add grid plot to the PlotPanel
		add_scene(hosts,flag);
	}
	
	public Plot3DPanel getPlot() {
		return this.plot;
	}

	public void print(double t, double[] y) {

		// add data point to the plot
		if (step < XYZ.length) {
			XYZ[step][0] = y[0];
			XYZ[step][1] = y[1];
			XYZ[step][2] = y[2];
		/*	XYZ1[step][0] = y[0];
			XYZ1[step][1] = y[1];
			XYZ1[step][2] = y[2];*/
			if (y[0] > max)
				max = y[0];
			if (y[1] > max)
				max = y[1];
			if (y[2] > max)
				max = y[2];

			step++;
		}
		// also print to the screen for warm fuzzy feeling
		// System.out.println(t + " " + y[0] + " " + y[1]);
	}
	
	public void print1(double t, double[] y) {

		// add data point to the plot
		if (step < XYZ1.length) {
		/*	XYZ[step][0] = y[0];
			XYZ[step][1] = y[1];
			XYZ[step][2] = y[2];*/
			XYZ1[step][0] = y[0];
			XYZ1[step][1] = y[1];
			XYZ1[step][2] = y[2];
			if (y[0] > max)
				max = y[0];
			if (y[1] > max)
				max = y[1];
			if (y[2] > max)
				max = y[2];

			step++;
		}
		// also print to the screen for warm fuzzy feeling
		// System.out.println(t + " " + y[0] + " " + y[1]);
	}
	
	public void print2(double t, double[] y) {

		// add data point to the plot
	/*	if (step < XYZ2.length) {
			XYZ[step][0] = y[0];
			XYZ[step][1] = y[1];
			XYZ[step][2] = y[2];
			XYZ2[step][0] = y[0];
			XYZ2[step][1] = y[1];
			XYZ2[step][2] = y[2];
			if (y[0] > max)
				max = y[0];
			if (y[1] > max)
				max = y[1];
			if (y[2] > max)
				max = y[2];

			step++;
		}*/
		// also print to the screen for warm fuzzy feeling
		// System.out.println(t + " " + y[0] + " " + y[1]);
	}

	/*
	 * public static void main(String[] args) {
	 * 
	 * 
	 * // put the PlotPanel in a JFrame like a JPanel JFrame frame = new
	 * JFrame("a plot panel"); frame.setSize(600, 600);
	 * frame.setContentPane(plot); frame.setVisible(true);
	 * 
	 * }
	 */
	public Function getFunction() {
		return func;
	}

}