...
public class TwoBody implements Derivatives
{
	...
	public double ta;
	/** default: earth GM in km^3/s^2 (value from JGM-3)*/
	protected double mu = 398600.4415;
	/** step length*/
	...
	/** 
	 * Construct a TwoBody orbit from 6 orbit elements 
	 * and set mu.Angles are input in degrees.
	 * @param k Kepler elements
	 * @param mu 
	 */
	...
	private void rv2Elements(VectorN r, VectorN v)
	{
		...
		// equatorial orbit, things blow up
		if (i == 0.0) 
		{
	 ...
	 */
	public void propagate1(double t0, double tf, Printable pr, boolean print_switch)
	{
		double[] temp = new double[6];
	...
	/**
	 * Propagate a TwoBody orbit of tf moment using Kepler's equation.
	 * @param tf current time in s.
	 * @throws IOException If file fails to open.
	 */
	public void propagate(double tf)
	{
		...
		double period = this.period();
		double dt = period / steps;
		// check to see if we're going past tf
		if ((t0 + dt) > tf) // check to see if we're going past tf
		{
			dt = tf - t0;
		}
