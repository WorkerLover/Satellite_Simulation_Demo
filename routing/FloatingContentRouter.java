...
public class FloatingContentRouter extends ActiveRouter {
	...
	private Coord  gpsLocation = new Coord (-1,-1,-1);
	...
	public void update() {
		...
		if (location_source == LOC_SRC_NONE) {
		    ...
		    if (n > 0) {
			last_known_location = loc = new Coord (x / (double) n, y / (double) n, z / (double) n);
			location_valid = true;
		    }
		}
    	...
    	public Coord getLocationWithError(double min, double max, double interval, int seed){
    		...
    		if (max > 0) {
    		    ...
    		    gpsLocation.setLocation (location.getX()+Math.cos(angle)*d, 
    					     location.getY()+Math.sin(angle)*d,location.getZ());
    		} else {
    		    gpsLocation.setLocation (location.getX(), location.getY(), location.getZ());
    		}
