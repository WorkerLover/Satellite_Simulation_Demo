...
public class SimScenario implements Serializable {
	...
	public static final String DEFAULT_NUMBER_S = "defaultNumbers";
	/** number of hosts in the group -setting id ({@value})*/
	public static final String RANDOM_NUMBER_S = "randomNumbers";
	/** number of hosts in the group -setting id ({@value})*/
	public static final String USER_CONFIG_NUMBER_S = "userConfigNumbers";
	/** number of hosts in the group -setting id ({@value})*/
	 ...
	 */
	protected void createHosts(Configuration config) {
		...
		for (int i=1; i<=nrofGroups; i++) {
			...
			int defaultNumbers = s.getInt(DEFAULT_NUMBER_S);
			int randomNumbers = s.getInt(RANDOM_NUMBER_S);
			int userConfigNumbers = s.getInt(USER_CONFIG_NUMBER_S);
			...
			for (int j=0; j<randomNumbers; j++) {
				...
				DTNHost host = new DTNHost(this.messageListeners, 
						...
						mmProto, mRouterProto,j,false,true);
				hosts.add(host);
			}
			
			for (int j=0; j<defaultNumbers; j++) {
				ModuleCommunicationBus comBus = new ModuleCommunicationBus();
				// prototypes are given to new DTNHost which replicates
				// new instances of movement model and message router
				DTNHost host = new DTNHost(this.messageListeners, 
						this.movementListeners,	gid, interfaces, comBus, 
						mmProto, mRouterProto,j,true,false);
				hosts.add(host);
			}
			
			for (int j=0; j<userConfigNumbers; j++) {
		//		System.out.println("user_config_numbers "+ config.user_config_numbers);
				ModuleCommunicationBus comBus = new ModuleCommunicationBus();
				// prototypes are given to new DTNHost which replicates
				// new instances of movement model and message router
				DTNHost host = new DTNHost(this.messageListeners, 
						this.movementListeners,	gid, interfaces, comBus, 
						mmProto, mRouterProto,config.parameter);
				hosts.add(host);
			}
